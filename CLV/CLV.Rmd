...
```{r gross_sales, echo=FALSE, warning=FALSE}
...
##### La customer value moyenne à 6 mois de nos clients est donc `r round(gross_sales_cli,2)` €
...
allcli <- commandes %>%
  ...
  summarise(orders_count = first(orders_count),
            orders_count18 = ifelse(min(age > 18),NA,max(nieme[age <= 18])),
            premiere = first(first_order_date),
            derniere = max(order_created_at),
            ...
            panier_moyen = mean(gross_sale),
            # Valeur du client, calculable seulement pour les clients dont on a toutes les commandes : clients arrivés après le 1er mai 17
            value = ifelse(premiere >= as.Date("2017-05-01"), sum(gross_sale), NA),
            value18 = ifelse(premiere >= as.Date("2017-05-01"), sum(gross_sale[age <= 18]), NA),
            ...
            actif18 = ifelse(actif & ddv < 18*30, NA, !(!actif & ddv < 18*30)))
...
t <- table(df$classe_count,df$actif)
...
affich <- data.frame(a = text, b = paste(round(100*res$probs,1),"%"))
...
Pour compléter la case manquante on va changer de méthode, ces clients étant encore actifs pour la grande part, on ne connait pas leur valeur effective.
...
Et grâce au panier moyen des clients de cette catégorie on peut déduire leur CLV moyenne.

```{r, echo=FALSE}
res$value_moy[nrow(res)] <- value
kable(res)
```

```{r, echo=FALSE}
clv <- sum(res$probs*res$value_moy)
```

Enfin, en calculant une moyenne pondérée on trouve une Customer Lifetime Value de `r round(clv,2)` €

## Analyse de survie

```{r, echo=FALSE}
# dfs <- df
# dfs$surv_orders <- Surv(dfs$orders_count, !dfs$actif)
# 
# km_orders <- survfit(dfs$surv_orders~1)
# 
# # survest <- stepfun(km$time, c(1, km$surv))
# 
# ggsurvplot(km_orders,
#            data = dfs,
#            censor = T,
#            censor.size = 3,
#            xlab = "Nombre de commandes",
#            ylab = "Chances qu'un client soit toujours actif",
#            break.x.by = 10,
#            ggtheme = theme_bw(),
#            risk.table = T,
#            fontsize = 3.8)
...
```{r,echo=FALSE}
delai1 <- function(df, x){
  return(mean(df$delai[df$nieme >= x], na.rm = T))
}
delai2 <- function(df, x){
  return(mean(df$delai[df$nieme == x], na.rm = T))
}

x <- 1:20
y <-sapply(x, delai1, df = comm)
y2 <- sapply(x, delai2, df = comm)

ggplot() +
  geom_point(mapping = aes(x = x, y = y)) +
  geom_point(mapping = aes(x = x, y = y2), color = "red") +
  theme_bw() +
...
```{r, echo=FALSE}
# On prend les clients partis ou immortels
ech <- allcli %>%
  filter(!actif | orders_count >= 6) %>%
  select(client_id)

df <- comm %>%
  filter(client_id %in% ech$client_id & orders_count > 1) %>%
  group_by(client_id) %>%
  summarise(delai2 = delai[nieme == 1],
            orders_count = first(orders_count),
            panier = gross_sale[nieme == 1]) %>%
  ungroup()

ggplot(df, aes(x = orders_count, y = delai2)) +
  geom_hline(yintercept = 30, linetype = "dashed", color = rouge) +
  geom_vline(xintercept = 5.5, linetype = "dashed", color = rouge) +
  geom_point(size = 1) +
  theme_bw() +
  scale_x_continuous(breaks = seq(0,40,2)) +
  scale_y_continuous(breaks = seq(0,250,20)) +
  labs(title = "Délai entre les 2 premières commandes selon le nombre total de commandes",
       # subtitle = "Pour les clients partis ou ayant atteint 10 commandes",
       x = "Nombre actuel de commandes",
       y = "Nombre de jours entre la 1ère et la 2nde commande")
